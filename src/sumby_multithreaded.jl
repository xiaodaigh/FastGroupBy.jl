...
function sumby_multi_rg{T, S<:Number}(by::AbstractVector{T},  val::AbstractVector{S})::Dict{T,S}
...
function sumby_multi_van{T, S<:Number}(by::AbstractVector{T},  val::AbstractVector{S})::Dict{T,S}
    (starts, ends) = blockranges(nthreads(), length(by))
    res = Dict{T,S}[]
    @threads for (ss, ee) in collect(zip(starts, ends))
        # @inbounds byv = @view by[ss:ee]
